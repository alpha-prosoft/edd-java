...
public interface Context {
  ...
  <A extends Aggregate> Optional<A> getAggregate(UUID aggregateId, Class<A> type);
   ...
   * this; a query without one uses {@link #getAggregate(UUID, Class)} (latest). The service is
   * responsible for keeping historical aggregate shapes readable.
   */
  <A extends Aggregate> Optional<A> getAggregate(UUID aggregateId, long version, Class<A> type);
}
