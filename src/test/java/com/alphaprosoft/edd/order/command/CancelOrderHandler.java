...
public final class CancelOrderHandler implements CommandHandler<CancelOrder, OrderAggregate> {

    @Override
    public HandlerResult<OrderAggregate> handle(Context ctx, CancelOrder cmd) {
        OrderAggregate order = ctx.get(OrderDeps.CURRENT_ORDER);
        return switch (order.status()) {
            case PLACED, PAID -> HandlerResult.of(new OrderCancelled(cmd.orderId(), cmd.reason()));
