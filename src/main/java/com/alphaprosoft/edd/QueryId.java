...
public final class QueryId<Q extends Query, R> {
    ...
    public static <Q extends Query, R> QueryId<Q, R> of(String id, Class<Q> queryType, Class<R> responseType) {
        QueryId<?, ?> existing = BY_ID.get(id);
        if (existing != null) {
            if (!existing.queryType.equals(queryType) || !existing.responseType.equals(responseType)) {
                throw new IllegalStateException("QueryId '" + id + "' already registered with different types");
            }
